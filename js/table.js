    ...
    <table>
      <thead>
        ...
        <tr>
          ...
          <th>t</th><th>${t('table.subHeaders.stops')}</th><th>${t('table.subHeaders.cond')}</th><th>${t('table.subHeaders.total')}</th><th>Σt</th><th>${t('table.subHeaders.remaining')}</th>
        </tr>
      </thead>
      <tbody>
  `;
      ...
      <tr>
        ...
        <td class="leg-cell editable-cell">
          <span class="leg-name" contenteditable="true" data-legkey="${L.key}" spellcheck="false"
                title="${escapeHtml(t('table.titles.editName'))}">${escapeHtml(displayLabel)}</span>
        </td>

        <td class="critical-cell ${isCritical ? 'critical-on' : ''}">
          <label class="crit-wrap" title="${escapeHtml(t('table.titles.markCritical'))}">
            <input type="checkbox"
                  class="wb-critical"
                  data-legkey="${L.key}"
                  ${isCritical ? 'checked' : ''} />
        ...
        <td class="editable-cell">
          <span class="wb-stops" contenteditable="true" data-legkey="${L.key}" spellcheck="false"
                title="${escapeHtml(t('table.titles.stops'))}">${escapeHtml(minutesToText(L.stopsMin))}</span>
        </td>
        <td class="editable-cell">
          <span class="wb-cond" contenteditable="true" data-legkey="${L.key}" spellcheck="false"
                title="${escapeHtml(t('table.titles.cond'))}">${escapeHtml(percentToText(L.condPct))}</span>
        </td>
        ...
        <td class="obs-cell editable-cell">
          <span class="wb-obs" contenteditable="true" data-legkey="${L.key}" spellcheck="true"
                title="${escapeHtml(t('table.titles.observations'))}">${escapeHtml(obsText)}</span>
        </td>
      </tr>
    `;
  }
...
export function exportRoadbooksCsv() {
  ...
  if (!table) { alert(t('alerts.noTable')); return null; }
